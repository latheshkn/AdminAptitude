...
public class SppinerAdapterForSubCatery extends ArrayAdapter<ModelFroSubcategory> {

    public SppinerAdapterForSubCatery(@NonNull Context context, int resource, @NonNull List<ModelFroSubcategory> objects) {
    ...
    private View inatlizeView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null)// only in this case we want to inflate
        {
            convertView = LayoutInflater.from(getContext())
                    .inflate(R.layout.spinneritemforsubcategory, parent, false);
        }

        ImageView country_flag = (ImageView) convertView.findViewById(R.id.img_image);
        TextView country_name = (TextView) convertView.findViewById(R.id.sub_Category);

//            country_name.setTextSize(15);
//            country_flag.getLayoutParams().height = 60;
//            country_flag.getLayoutParams().width = 60;
        ModelFroSubcategory countryItem = getItem(position);
...
//                country_flag.setImageResource(countryItem.;
            country_name.setText(countryItem.getSubCategory_name());
