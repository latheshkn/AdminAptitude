...
public class SpinnerAdapterSelectType extends ArrayAdapter<ModelClassForSpinnerSelectType> {

    public SpinnerAdapterSelectType(@NonNull Context context, int resource, @NonNull List<ModelClassForSpinnerSelectType> objects) {
    ...
    private View inatlizeView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null)// only in this case we want to inflate
        {
            convertView = LayoutInflater.from(getContext())
                    .inflate(R.layout.select_type_spinner_item, parent, false);
        }
...
//            country_flag.getLayoutParams().width = 60;
        ModelClassForSpinnerSelectType countryItem = getItem(position);
...
//                country_flag.setImageResource(countryItem.;
            country_name.setText(countryItem.getType_name());
